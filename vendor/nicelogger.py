...
class TornadoLogFormatter(logging.Formatter):
    def __init__(self, color, *args, **kwargs):
        ...
        if color:
            ...
            if sys.hexversion < 0x30203F0:
                fg_color = str(
                    curses.tigetstr("setaf") or curses.tigetstr("setf") or "", "ascii"
                )
            else:
            ...
            self._colors = {
                ...
                logging.CRITICAL: str(curses.tparm(fg_color, 9), "ascii"),  # Bright Red
            }
    ...
    def format(self, record):
        ...
        prefix = (
            "[%(levelname)1.1s %(asctime)s.%(msecs)03d %(module)s:%(lineno)d]"
            % record.__dict__
        )
        if self._color:
            prefix = (
