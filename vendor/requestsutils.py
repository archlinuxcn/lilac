...
def download_into(
    session: requests.Session, url: str, file: BinaryIO, process_func=None
) -> None:
...
def download_into_with_progressbar(url, dest):
    ...
    with open(dest, "wb") as f:
        download_into(
            requests, url, f, partial(download_process, dest, time.time(), width=w)
        )
...
class RequestsBase:
    ...
    def request(
        self, url: str, method: Optional[str] = None, *args, **kwargs
    ) -> requests.Response:
        if self.baseurl:
