...
def git_pull_override() -> bool:
    try:
        ...
        try:
            ...
            pass
        output = run_cmd(["git", "pull", "--no-edit"], env=env,)
    except subprocess.CalledProcessError as e:
        if "would be overwritten by merge:" in e.output:
            files = [
                line.strip() for line in e.output.splitlines() if line.startswith("\t")
            ]
...
def run_cmd(
    ...
    *,
    ...
    logger.debug(
        "running %r, %susing pty,%s showing output",
        cmd,
        "" if use_pty else "not ",
        " not" if silent else "",
    )
    ...
    try:
        ...
        p = subprocess.Popen(
            cmd, stdin=stdin, stdout=stdout, stderr=subprocess.STDOUT, cwd=cwd, env=env,
        )
        if use_pty:
            os.close(stdout)
        else:
