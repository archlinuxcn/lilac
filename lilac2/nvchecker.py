...
def _gen_config_from_mods(mods: LilacMods,) -> Tuple[Dict[str, Any], Set[str]]:
    unknown = set()
    newconfig = {}
...
def packages_need_update(
    ...
    for pkg in unknown:
        ...
        for maintainer in maintainers:
            error_owners[maintainer].append(
                {"name": pkg, "event": "package without `update_on` config",}
            )

    for who, their_errors in error_owners.items():
        logger.warning(
            "send nvchecker report for %r packages to %s",
            {x["name"] for x in their_errors},
            who,
        )
        repo.sendmail(
    ...
    for name, d in nvdata_unord.items():
        ...
        for i, (j, nr) in enumerate(sorted(d.items())):
            if i != j:
                logger.warning(
                    "mismatched nvdata_unord item for %s: %d != %d in %r", name, i, j, d
                )
...
def nvtake(L: Iterable[str], mods: LilacMods) -> None:
    ...
    run_cmd(
        ["nvtake", "--ignore-nonexistent", NVCHECKER_FILE]  # type: ignore
        + names
    )  # type: ignore
