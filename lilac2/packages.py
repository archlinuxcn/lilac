...
_DependencyTuple = namedtuple("_DependencyTuple", "pkgdir pkgname")
...
class Dependency(_DependencyTuple):
    ...
    def resolve(self) -> Optional[Path]:
        ...
        if len(pkgs) == 1:
            ...
            ret = sorted(pkgs, reverse=True, key=lambda x: x.stat().st_mtime)[0]
            return ret
...
class DependencyManager:
    ...
    def get(self, what: Union[str, Tuple[str, str]]) -> Dependency:
        ...
        if key not in self._CACHE:
            self._CACHE[key] = Dependency(self.repodir / pkgbase, pkgname)
