...
class Repo:
    ...
    def __init__(self, config: configparser.ConfigParser):
        ...
        self.trim_ansi_codes = not config.getboolean("smtp", "use_ansi", fallback=False)
    ...
    def find_maintainers(self, mod: LilacMod) -> List[Maintainer]:
        ...
        if maintainers is not None:
            for m in maintainers:
                if "github" in m and "email" in m:
                    ret.append(Maintainer.from_email_address(m["email"], m["github"]))
                elif "github" in m:
                    try:
        ...
        if not ret:
            logger.warning(
                "lilac doesn't give out maintainers for %s, " "fallback to git.",
                mod.pkgbase,
            )
    ...
    def find_maintainer_by_git(
        ...
        p = subprocess.Popen(
            cmd, stdout=subprocess.PIPE, universal_newlines=True, cwd=dir,
        )
    ...
    def send_error_report(
        ...
        *,
        ...
        logger.debug(
            "mail to %s:\nsubject: %s\nbody: %s", addresses, subject_real, msg[:200]
        )
        self.sendmail(addresses, subject_real, msg)

    def sendmail(
        self, who: Union[str, List[str], Maintainer], subject: str, msg: str
    ) -> None:
    ...
    def load_managed_lilac_and_report(self) -> Set[str]:
        ...
        for name, exc_info in errors.items():
            ...
            self.send_error_report(
                name, exc=(exc, tb), subject="为软件包 %s 载入 lilac.py 时失败"
            )
