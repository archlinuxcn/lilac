...
class MailService:
    def __init__(self, config) -> None:
        ...
        self.unsub = config.get("lilac", "unsubscribe_address", fallback=None,)
    ...
    def sendmail(self, to: Union[str, List[str]], subject: str, msg: str) -> None:
        ...
        if len(msg) > 5 * 1024 ** 2:
            msg = msg[: 1024 ** 2] + "\n\n日志过长，省略ing……\n\n" + msg[-(1024 ** 2) :]
        mail = assemble_mail(
            "[%s] %s" % (self.mailtag, subject), to, self.from_, text=msg
        )
        if self.unsub:
