    ...
    try:
        ...
        run_cmd(
            [
                "sh",
                "-c",
                "rm -f -- *.pkg.tar.xz *.pkg.tar.xz.sig *.pkg.tar.zst *.pkg.tar.zst.sig",
            ]
        )
        pre_build = getattr(mod, "pre_build", None)
        if pre_build is not None:
            logger.debug(
                "accept_noupdate=%r, oldver=%r, newver=%r",
                accept_noupdate,
                oldver,
                newver,
            )
        ...
        build_prefix = build_prefix or getattr(mod, "build_prefix", "extra-x86_64")
        depend_packages = []
    ...
    if build_prefix == "makepkg":
        ...
        for b in bindmounts:
            ...
            if not os.path.exists(source_dir):
                logger.warning(
                    "Invalid bindmount spec %s: " "source dir does not exist", b
                )
                continue
            cmd += ["-d", b]
