...
def obtain_array(name: str) -> Optional[List[str]]:
  ...
  """
    with tempfile.NamedTemporaryFile() as output_file:
        command_write_array_out = """printf "%s\\0" "${{{}[@]}}" > {}""".format(
            name, output_file.name
        )
        command_export_array = [
            "bash",
            "-c",
            "source PKGBUILD && {}".format(command_write_array_out),
        ]
        subprocess.run(command_export_array, stderr=subprocess.PIPE, check=True)
...
def obtain_optdepends(
    ...
    if parse_dict:
        return {
            pkg.strip(): desc.strip()
            for (pkg, desc) in (item.split(":", 1) for item in obtained_array)
        }
    else:
        return obtained_array
...
def vcs_update(*, keep_version: bool = False) -> None:
    ...
    if pkgver and pkgver == new_pkgver and pkgrel == new_pkgrel:
        # update for rebuild
        update_pkgrel()
...
def update_pkgver_and_pkgrel(newver: str, *, updpkgsums: bool = True) -> None:
...
def update_pkgrel(rel: Optional[PkgRel] = None,) -> None:
    with open("PKGBUILD") as f:
        pkgbuild = f.read()
...
def git_add_files(files: Union[str, List[str]], *, force: bool = False,) -> None:
...
def git_commit(*, check_status: bool = True) -> None:
    if check_status:
        ret = [
            x
            for x in run_cmd(["git", "status", "-s", "."]).splitlines()
            if x.split(None, 1)[0] != "??"
        ]
        if not ret:
            return
...
def _update_aur_repo_real(pkgname: str) -> None:
    ...
    with at_dir(aurpath):
        with open(".SRCINFO", "wb") as srcinfo:
            subprocess.run(
                ["makepkg", "--printsrcinfo"], stdout=srcinfo, check=True,
            )
        run_cmd(["git", "add", "."])
        run_cmd(
            [
...
def update_aur_repo() -> None:
    ...
    try:
        ...
        _G.repo.send_error_report(
            _G.mod, exc=(e, tb), subject="提交软件包 %s 到 AUR 时出错",
        )
...
def single_main(build_prefix: str = "makepkg") -> None:
    ...
    with lilacpy.load_lilac(Path(".")) as mod:
        lilac_build(
            mod, None, build_prefix=build_prefix, accept_noupdate=True,
        )
...
def download_official_pkgbuild(name: str) -> List[str]:
    ...
    tree_url = (
        "https://projects.archlinux.org/svntogit/%s.git/tree/repos/%s-%s?h=packages/%s"
    ...
    for filename in files:
        blob_url = (
            "https://projects.archlinux.org/svntogit/%s.git/plain/repos/%s-%s/%s?h=packages/%s"
