...
def gen_pkgbuild(
    ...
    source_release = [x for x in j["releases"][version] if x["packagetype"] == "sdist"]
    if not source_release:
        whl_release = [
            x for x in j["releases"][version] if x["packagetype"] == "bdist_wheel"
        ]
        if not whl_release:
    ...
    if makedepends2:
        depends_str.append(f"makedepends=({to_sharray(makedepends2)})")
    if optdepends:
        depends_str.append(f"optdepends=({to_sharray(optdepends)})")
...
"""
        if license_file:
            package_code += pkg_license_tmpl.format(license_file=license_file)
